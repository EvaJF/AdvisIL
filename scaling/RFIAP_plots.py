...
input_csv = cp["input_csv"]  #"/home/eva/Documents/code/rfiap_plots/results_MS.csv"
dataset = cp["dataset"] 
prefix = cp["prefix"] 
ntot_states = cp["ntot_states"] 
ntot_classes = cp["ntot_classes"] 
epochs_init = cp["epochs_init"] 
epochs_incr = cp["epochs_incr"] 
algo = cp["algo"] 
...
### Plot funcitons : Précision en fonction du nombre de paramètres ###

def plot_acc_nparams() : 
    fig, ax = plt.subplots()
    box = ax.get_position()
    ax.set_position([box.x0, box.y0+0.02, box.width, box.height*1.1]) # occupy a bit more space (crop upper margin for title)
    marker = ['-', '-.', '--']
    i=0
    for b in set(results_df.backbone) :
        temp_df = results_df[(results_df.backbone==b) & (results_df.n_params < 6e6)]
        temp_df.sort_values(by=['n_params'], inplace=True)
        ax.plot(temp_df['n_params'].to_list(), temp_df['acc5'].to_list(), marker[i], linewidth=4, label=b)
        i+=1
    # Comments and labels
    ax.legend(loc='center left', bbox_to_anchor=(0.48, 0.3), fontsize=17) # , title='Backbone', loc="lower right", loc='center left', bbox_to_anchor=(1, 0.5)
    font = {'family':'serif','color':'black','size':18}
    ax.set_xlabel("nombre de paramètres", fontdict = font)
    ax.set_ylabel("précision top 5 en %", fontdict = font)
    plt.xticks(fontsize=16) 
    plt.yticks(fontsize=16) 
    # Save fig    
    save_path = os.path.join(save_dir, prefix + 'acc_nparams.png')
    plt.savefig(save_path)
    print("Saved figure under %s" %save_path)
    return None

plot_acc_nparams()
### Plot functions : Précision Top 5 en fonction du Coefficient de largeur  ###
...
def plot_v1(): 
    for b in set(results_df.backbone): 
        ...
        plt.xticks(fontsize=16) 
        plt.yticks(fontsize=16) 
        plt.savefig(os.path.join(save_dir, prefix + 'acc_width_v1_'+b+'.png'))
...
def my_plot(b, mem, marker, mem_label, input_df):
    ...
    for i in range(len(marker)):
        ...
        ax.annotate('  '+str(round(h_df['n_params'].tolist()[0]/1e6, 1))+'e6', (h_df['w_mult'].tolist()[0], h_df['acc5'].tolist()[0]), fontsize=14) 
    # plot reference model
    ref_df = results_df[(results_df.backbone==b) & (results_df.w_mult==1.0) & (results_df.d_mult==1.0)]
...
for b in set(results_df.backbone): 
    ...
    #plot_v1()
    my_plot(b, mem=mem, marker=marker, mem_label=mem_label, input_df=results_df) # regular
    #my_plot2(b, mem=mem, marker=marker, mem_label=mem_label, input_df=results_df) # filled
