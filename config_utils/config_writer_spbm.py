...
exists_val = False """.format(backbone=backbone, width_mult=width_mult, depth_mult=depth_mult,
    ...
    lr = lr, train_file_path=train_file_path, test_file_path=test_file_path,
...
conda activate py37
nvidia-smi 
srun --nodes=1 --ntasks=1 --gres=gpu:1 python /home/users/efeillet/incremental-scaler/SPBM/codes/main.py {config_path}/spbm.cf 
...
""".format(error_path=error_path, log_path=log_path, partition=partition_gpu, config_path=config_path) 
        ...
        # append echo
        single_launcher+="""echo EXPE /home/users/efeillet/incremental-scaler/SPBM/codes/main.py {config_path}/spbm.cf\n""".format(
            config_path=config_path) 
        # append srun
        single_launcher+="""srun --nodes=1 --ntasks=1 --gres=gpu:1 python /home/users/efeillet/incremental-scaler/SPBM/codes/main.py {config_path}/spbm.cf\n""".format(
            config_path=config_path)
...
if yaml_folder_path != "None": # a folder containing multiple yaml files is provided
    for filename in [filename for filename in os.listdir(yaml_folder_path) if filename.endswith(".yaml") and "spbm" in filename]:
        print("\n Yaml file : ", filename)
        u_prefix = filename.split(".")[0] # ex : imagenet_random_0_siw_equi_s10
        u_dataset = '_'.join(u_prefix.split('_')[:-4])
