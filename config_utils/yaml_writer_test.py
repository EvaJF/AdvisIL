...
### different settings for quick testing ###

# where to store yaml files
destination_path = "/home/data/efeillet/expe/test"
if not os.path.exists(destination_path):
    os.makedirs(destination_path)
if not os.path.exists(os.path.join(destination_path, 'lucir')):
    os.makedirs(os.path.join(destination_path, 'lucir'))
if not os.path.exists(os.path.join(destination_path, 'siw')):
    os.makedirs(os.path.join(destination_path, 'siw'))
if not os.path.exists(os.path.join(destination_path, 'spbm')):
    os.makedirs(os.path.join(destination_path, 'spbm'))

# scenarii : (prefix, total number of states, number of classes int he first batch, number of classes per incremental state).
state_list = [("equi_s5_B20_P20", 5, 20, 20),  ("semi_s5_B50_P10", 5, 50, 10), ("semi_s3_B50_P25", 3, 50, 25)]

for (prefix, last_batch_number, B, P) in state_list : 
    assert(B+(last_batch_number-1)*P == 100)
    assert(prefix.endswith(str(last_batch_number)))
    if B == P : 
        assert(prefix.startswith("equi"))
    else : 
        assert(prefix.startswith("semi"))
        
# dataset list : random subsets and thematic subsets from ImageNet, each contains 100 classes with 400 images (train+test)
dataset_list = ["imagenet_random_0"]
...
for dataset in dataset_list: 
  ...
  subsequent_epochs : 2 # incr \n\n""".format(**config_dic_i)
            i+=1

        ## SAVE FILE
        if not os.path.exists(os.path.join(destination_path, "lucir")):
            os.makedirs(os.path.join(destination_path, "lucir"))
            print("\n\nCreated folder %s" %os.path.join(destination_path, "lucir"))
...
for dataset in dataset_list: 
  ...
  subsequent_epochs : 2 # incr \n\n""".format(**config_dic_i)
        ...
        if not os.path.exists(os.path.join(destination_path, "spbm")):
            os.makedirs(os.path.join(destination_path, "spbm"))
            print("\n\nCreated folder %s" %os.path.join(destination_path, "spbm"))
...
for dataset in dataset_list: 
  ...
  subsequent_epochs : 2 # incremental states\n\n""".format(**config_dic_i)
        ...
        if not os.path.exists(os.path.join(destination_path, "siw")):
            os.makedirs(os.path.join(destination_path, "siw"))
            print("\n\nCreated folder %s" %os.path.join(destination_path, "siw"))
