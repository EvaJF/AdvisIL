...
##############
### Config ###
output_dir = "/home/efeillet/Code/AdvisIL/"

##################
### Input data ###
input_df = pd.read_csv("/home/efeillet/Code/AdvisIL/advisIL_avg_logs_ref.csv")
print("Input Length   : ", len(input_df))

## Discard results from rd2 dataset
input_df = input_df[input_df.dataset!='imagenet_random_2']
print("Length w/o Rd2 : ", len(input_df))
...
for dataset in input_df.dataset.unique() : 
    ...
    fig.tight_layout()
    out_png = os.path.join(output_dir, "ref_"+dataset+"_heatmap_scenario+budget.png")
    plt.savefig(out_png)
...
for i in range(len(y_labels)):
    for j in range(len(x_labels)):
        text = ax.text(j, i, data[i, j], ha="center", va="center", color="w")
ax.set_title("All 5 reference datasets"+"\n# selections as best {method, model_name} over\n all scenarii+budget; criterion : top1 acc")
fig.tight_layout()
out_png = os.path.join(output_dir, "ref_data5_heatmap_scenario+budget.png")
plt.savefig(out_png)
print("Saved fig under "+out_png)
...
for i in range(len(y_labels)):
    for j in range(len(x_labels)):
        text = ax.text(j, i, data[i, j], ha="center", va="center", color="w")
ax.set_title("# selections as best {method, model_name} over all scenarii \ntop1 acc averaged over 5 reference datasets")
fig.tight_layout()
out_png = os.path.join(output_dir, "ref_heatmap_scenario.png")
plt.savefig(out_png)
print("Saved fig under "+out_png)
...
fig.tight_layout()
out_png = os.path.join(output_dir, "ref_heatmap_scenario+budget.png")
plt.savefig(out_png)
print("Saved fig under "+out_png)
...
# best individual : choix par défaut en prenant la combinaison méthode + backbone la meilleure en moyenne sur tous les datasets de référence, pour ce budget et ce scénario
