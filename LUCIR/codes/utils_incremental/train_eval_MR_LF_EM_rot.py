...
def train_eval_SPB_rot(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            targets = torch.cat([targets for _ in range(4)]).to(device)
            inputs=torch.cat([torch.rot90(inputs, k+1, [2, 3]) for k in range(4)]).to(device)
