...
# TODO adapt regex to >>> results and EXPE ...
for path in log_list : 
    with open(path) as f:
        lines = f.readlines()
    results = [l for l in lines if l.startswith(">>> results")]
    expe = [l for l in lines if l.startswith("EXPE")]
    print("\nResults : ", len(results)) 
    print(results)
    print("\nEXPE : ", len(expe))
    print(expe)
    assert(len(results)==len(expe))
    for i in range(len(results)) : 
        try : 
            acc1.append(float(results[i].split()[-4]))
            acc5.append(float(results[i].split()[-1]))
            backbone.append(expe[i].split()[-1].split('/')[-4].split('_')[0])
            w_mult.append(float(re.findall(r"[-+]?\d*\.*\d+",expe[i].split()[-1].split('/')[-4].split('_')[1])[0]))
            d_mult.append(float(re.findall(r"[-+]?\d*\.*\d+",expe[i].split()[-1].split('/')[-4].split('_')[2])[0]))
            print(acc1[-1], acc5[-1], backbone[-1], w_mult[-1], d_mult[-1])
        except:
            acc1.append(0.0)
            acc5.append(0.0)
            backbone.append("unknown")
            w_mult.append(0.0)
            d_mult.append(0.0)
...
dic = {'dataset':dataset, 'ntot_states':ntot_states, 'ntot_classes':ntot_classes, 
'epochs_init':epochs_init, 'epochs_incr':epochs_incr, 'algo':algo, 
'backbone':backbone, 'w_mult':w_mult, 'd_mult':d_mult, 'n_params':n_params, 
'acc1':acc1, 'acc5':acc5}
for key in dic.keys() :
    print("Key {key} : length {length}".format(key=key, length=len(dic[key])))
