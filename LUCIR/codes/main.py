...
with warnings.catch_warnings(record=True) as warn_list:
    ...
    ################################################
    assert(B >= P)
    print("Number of classes in the first state : {} \nNumber of classes in the subsequent states {}.".format(B,P))
    ...
    print("prefix for checkpoints : ", ckp_prefix)
    if random_seed!=-1:
    ...
    for iteration_total in range(nb_runs):
        ...
        #take classes in the numerical order of their labels
        ...
        for b in range(first_batch_number, last_batch_number):
            ...
            if b == first_batch_number:
                ...
                out_features = tg_model.fc.out_features
                print("in_features:", in_features, "out_features:", out_features)
                new_fc = modified_linear.SplitCosineLinear(in_features, out_features, P) # // or
            ...
            if b > first_batch_number and imprint_weights:
                ...
                if backbone=="resnetBasic" or backbone=="resnetBottleneck":
                    ...
                    tg_feature_model = nn.Sequential(*list(tg_model.children())[:-1],nn.AdaptiveAvgPool2d((1, 1)))
                else : 
            ...
            ckp_name = os.path.join(ckp_dir,
                                    ckp_prefix+'_model_{}.pth'.format(b)) 
            ...
            #    torch.save(tg_model, ckp_name.replace("LUCIR", "LUCIR/AFTER_CBF_")) # .replace("/checkpoint/", "/checkpoint/AFTER_CBF_")
            #    torch.save(class_means, class_means_name.replace("LUCIR", "LUCIR/AFTER_CBF_")) # .replace("/checkpoint/", "/checkpoint/AFTER_CBF_")
        ...
        print('>>> results LUCIR_CNN | acc@1 = {:.2f} \t acc@5 = {:.2f} '.format(mean_top1_cnn_cumul_acc, mean_top5_cnn_cumul_acc))
