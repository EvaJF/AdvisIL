...
def compute_score(nb_batch):
    ...
    for c in range(first_batch_size+batch_size*nb_batch):
        with open(os.path.join(path_pred,str(c)), newline='') as f:
            ...
            to_append_top5 = [[int(elt[i].split(":")[0]) for i in range(1)] for elt in list(reader)]
...
print('>>> results MobIL | acc@1 = {el1:.3f} \t acc@5 = 0.0'.format(el1 = sum(top1[1:])/len(top1[1:]))) # \t acc@5 = {el2:.3f}, el2 = sum(top5[1:])/len(top5[1:])