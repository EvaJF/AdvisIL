...
def expe_writer(expe_dic, output_dir =  output_dir):
    ...
    expe0:
        ...
        datasets_mean_std_file_path : "/your/path/to/AdvisIL/images_list_files/datasets_mean_std.txt"
        ...
        test_file_path : /your/path/to/AdvisIL/images_list_files/train100/inat/test.lst  
    ...
    test_file_path = expe_dic["test_file_path"] # /your/path/to/AdvisIL/images_list_files/train100/mininat/test.lst
    ...
    first_model_name = ckp_prefix +"_model_0.pth"
    first_path_model = os.path.join(output_dir, normalization_dataset_name, 
                               ckp_prefix, model_name, "LUCIR", "models", 
                               first_model_name)
    print(first_path_model)    
    ## Write configs ###
    config_text = """[main.py]
############################### ARCHITECTURE
backbone = {backbone}
...
############################### pretrained initial model
first_path_model = {first_path_model}
############################### FIRST STATE 
first_train_batch_size       = 128 
first_base_lr                = 0.1 
first_lr_strat               = 20, 40 
first_lr_factor              = 0.1 
first_custom_weight_decay    = 0.0001 
first_epochs                 = {first_epochs} 
############################### SUBSEQUENT STATES 
subsequent_train_batch_size = 64 
subsequent_base_lr_features_extractor = 0.001 
subsequent_base_lr_classifier = 0.01 
subsequent_lr_strat = 20 
subsequent_lr_factor = 0.1 
subsequent_custom_weight_decay = 0.0001 
...
exists_val = False """.format(backbone=backbone, width_mult=width_mult, depth_mult=depth_mult,
    first_path_model = first_path_model, first_epochs=first_epochs, subsequent_epochs=subsequent_epochs,  
    ...
    for (text, filename) in [(config_text, "spbm.cf")]: 
...
conda activate py37
nvidia-smi 
srun --nodes=1 --ntasks=1 --gres=gpu:1 python /your/path/to/AdvisIL/SPBM/codes/main.py {config_path}/spbm.cf 
...
def single_launcher(expe_list, u_dataset, u_prefix, partition = partition_gpu, output_dir =  output_dir):
    """
    Single launcher file will be stored at output_dir/unique/single_launcher.sh
    ...
    # logs subdir for unified launcher 
...
exists_val = False """.format(backbone=backbone, width_mult=width_mult, depth_mult=depth_mult,
        ...
        for directory in [output_dir, os.path.join(output_dir, normalization_dataset_name),
            ...
            if not os.path.exists(directory):
                ...
                print("\n\nCreated folder %s" % directory)        # open and write files
        for (text, filename) in [(config_text, "spbm.cf")]: 
            print("\nCreating new file with path %s" %os.path.join(config_path, filename))
            text_file = open(os.path.join(config_path, filename), "w")   
            #write string to file
...
""".format(error_path=error_path, log_path=log_path, partition=partition_gpu, config_path=config_path) 
        ...
        # append echo
        single_launcher+="""echo EXPE /your/path/to/AdvisIL/SPBM/codes/main.py {config_path}/spbm.cf\n""".format(
            config_path=config_path) 
        # append srun
        single_launcher+="""srun --nodes=1 --ntasks=1 --gres=gpu:1 python /your/path/to/AdvisIL/SPBM/codes/main.py {config_path}/spbm.cf\n""".format(
            config_path=config_path)
