...
def train_eval_SPB_rot(epochs, first_path_model, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    top = 5
    if os.path.exists(first_path_model):
        print("reusing model", first_path_model)
        tg_model = torch.load(first_path_model)
        tg_model.avgpool = nn.AvgPool2d(7, stride=1)
        num_first_classes = tg_model.fc.out_features
        if num_first_classes < 5 : # handle scenarios with less than 5 classes in the initial state 
            top = num_first_classes
    else:
        tg_model.avgpool = nn.AvgPool2d(7, stride=1)
        if iteration > start_iteration:
            ref_model.eval()
            num_old_classes = ref_model.fc.out_features
            if num_old_classes < 5 : # handle scenarios with less than 5 classes in the initial state /+ first incremental state(s)
                top = num_old_classes
            else : 
                top = 5
            ...
            #print("\nBefore : list(tg_model.children()", type(tg_model), list(tg_model.children()))
            ...
            if backbone=="resnetBasic" or backbone=="resnetBottleneck":
                ...
                ref_feature_model = nn.Sequential(*list(ref_model.children())[:-2],nn.AdaptiveAvgPool2d((1, 1)))
            else : 
    ...
    print('state',iteration,'| Test ({}) |  acc@1 = {:.2f} | acc@{} = {:.2f}'.format(len(testloader), top1.avg, top, top5.avg))
