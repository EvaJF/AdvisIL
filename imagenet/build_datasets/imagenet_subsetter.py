...
class_dic={} # dict key = imagenet leaf id, value = number of images for this class
for line in class_list_file:
    line = line.split()
    imagenet_class_id = line[0]
    n_items = int(line[1])
    class_dic[imagenet_class_id] = n_items
print("There are a total of {} available classes.".format(len(class_dic.keys())))
#print(type(list(class_dic.keys())))
#print(list(class_dic.keys()))

# select a random num_classes classes 
class_subset = np.random.choice(list(class_dic.keys()), num_classes, replace=False)
#print(class_subset)
...
with open(train_list_file_path, 'w') as train, open(test_list_file_path, 'w') as test:
    ...
    for class_id in class_subset : 
        ...
        # sanity checks
        #print(image_list[:5])
        print("Number of images ", len(image_list), "/ Number of images as in dic : ", class_dic[class_id])
